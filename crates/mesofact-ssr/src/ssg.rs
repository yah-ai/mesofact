//! Build-time JS execution on deno_core (W174 pillar 4, SSG side). One V8
//! isolate per build hosts three jobs the Bun pipeline did in-process:
//! evaluating the bundled `mesofact.routes.ts`, probing SSR entrypoints'
//! default export, and rendering static/spa routes to HTML (the SSG pass,
//! R449-F1).
//!
//! Lives under `mesofact-ssr` (not `mesofact-build`) so the dev-tier SSR
//! runtime can share the module loader + harness boilerplate without
//! pulling in the build crate's heavyweight deps (W174 / R449-F2).
